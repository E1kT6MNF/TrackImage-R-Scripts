...
#     Rev. History:
      ...
      #     09/01/15 1. changed output form of results table from clipboard to .csv file
      #              2. added a header section to results table listing file source and parameters used.
#     Requirements:
...
source("S:\\OTCCommon\\AD_Reliability\\RAT\\Projects\\John\\R Scripts\\TrackImage Scripts\\2015.08.11 Cushion stability metrics.r")
...
header <- matrix(c(paste("Source =",res$Source),
                  paste("Test Quantity =",res$Quantity),
                  paste("Offset =", offset),
                  paste("LTL = ", LTL),
                  paste("UTL = ", UTL),
                  paste("ST1 = ", ST1),
...
my.write(dat= dat3, file = basename(res$Source), header = header) 
#
#     End